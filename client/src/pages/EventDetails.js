...
class EventDetails extends Component {
	...
	componentDidMount() {
		...
		axios.get(`http://localhost:7000/api/v1/event/${match.params.id}`).then((res) => {
	...
	render() {
		return (
			<div>
				...
				<Card style={{ margin: 'auto', width: '80%', marginTop: '10%', backgroundColor: '#fbe9e7' }}>
					...
					<Grid container style={{ margin: '5%' }}>
						<Grid container>
							...
							<Grid item xs={4}>
								<Typography variant="h4">Rp.{this.state.event.price * this.state.count}</Typography>
							</Grid>
						</Grid>
						<Grid container>
				...
				<Grid container style={{ margin: 'auto', width: '80%', marginTop: '10%', backgroundColor: '#fbe9e7' }}>
					<Grid item xs={6}>
					...
					<Grid item xs={6}>
						<div style={{ margin: 'auto', width: '90%' }}>
							...
							<div style={{ display: 'flex', justifyContent: 'center', margin: '5%' }}>
								<iframe src={`${this.state.event.urlmaps}`} width="300" height="300" />
							</div>
